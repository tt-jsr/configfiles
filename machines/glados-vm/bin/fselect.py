...
class Main(object):
    ...
    def Save(self):
       ...
       self.statuswin.Message("", "{0} Saved".format(self.configFile))
    ...
    def WriteSelectedFiles(self):
        file = None
        if self.useFilename == None:
            file = "/tmp/fselect.tmp"
        else:
            file = self.useFilename
        f  = open(file, "w")
        for o in self.selectedFilenames:
            f.write(o + "\n")
        f.close()
        return self.useFilename != None
...
if __name__ == "__main__":
    ...
    if configPath == None:
        ...
        configPath = "{0}/.config/fselect/fselect.dat".format(home)
