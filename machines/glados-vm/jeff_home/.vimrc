...
" => VIM user interface
"""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""
" scrolloff, Set 7 lines to the cursor - when moving vertically using j/k
"set so=7
...
if has("gui_running")
    "set guioptions-=T  "I want the toolbar
    set guioptions+=e
    set t_Co=256
    set guitablabel=%M\ %t
else
...
function SwitchProject(proj)
    bwipe
    let cmd = ":Project /home/jeff/" . a:proj . ".proj"
    exec cmd
endfunction
command -nargs=1 SwitchProject :call SwitchProject(<f-args>)

"==========================================================
"List todo and other comments
function Todo()
    let cmd = ':Rgrep "//[ ]*TODO[-a-zA-Z0-9]*:"'
    exec cmd
    cwindow
endfunction
...
command -nargs=1 Make :call Make(<f-args>)
 
" Do some things when vim is completely up and running
function VimStarted()
    "Check for "project.vim" file in the current or parents directory
    let s:projfile = findfile("project.vim", ".;")
    if filereadable(s:projfile)
        let cmd = "source " . s:projfile
        exec cmd
    endif
"   :NERDTree
    normal l
...
" viki config
"
"let g:vikiOpenFileWith_html="silent! chromium %{FILE}
